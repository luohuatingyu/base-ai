<template>
  <div class="panel docs-page">
    <section class="docs-intro" aria-labelledby="docs-intro-title">
      <div>
        <span class="docs-eyebrow">{{ t('workflowNodeDocs.handbook') }}</span>
        <h2 id="docs-intro-title">{{ t('workflowNodeDocs.guideTitle') }}</h2>
        <p>{{ t('workflowNodeDocs.description') }}</p>
      </div>
      <dl class="docs-stats" :aria-label="t('workflowNodeDocs.catalogOverview')">
        <div><dt>{{ rows.length }}</dt><dd>{{ t('workflowNodeDocs.templates') }}</dd></div>
        <div><dt>{{ documentedTypeCount }}</dt><dd>{{ t('workflowNodeDocs.nodeTypes') }}</dd></div>
        <div><dt>{{ filtered.length }}</dt><dd>{{ t('workflowNodeDocs.matches') }}</dd></div>
      </dl>
    </section>
    ...
    <div class="docs-layout">
      <aside class="docs-directory" :aria-label="t('workflowNodeDocs.directory')">
        ...
        <el-scrollbar class="docs-directory-scroll" max-height="calc(100vh - 330px)">
          <nav class="docs-node-list">
            <button v-for="item in filtered" :key="item.id" type="button" :class="{active:current?.id===item.id}"
              ...
              <small>{{ categoryLabel(item.functionalCategory) }} · {{ sourceLabel(item.source) }}</small>
            </button>
          </nav>
...
const documentedTypeCount=computed(()=>new Set(rows.value.map(item=>String(item.nodeType||'').toUpperCase())).size)
...
@media(max-width:800px){.docs-intro{align-items:flex-start;flex-direction:column;padding:22px}.docs-stats{width:100%;justify-content:space-between}.docs-layout{grid-template-columns:1fr}.docs-directory{border-right:0;border-bottom:1px solid var(--app-border)}.docs-directory-scroll{max-height:300px!important}.docs-content{padding:24px 20px 34px}.docs-example-grid{grid-template-columns:1fr}.docs-config-example{grid-column:auto}.docs-hero h2{font-size:24px}}
@media(max-width:480px){.docs-intro h2{font-size:21px}.docs-stats{gap:10px}.docs-content{padding-right:16px;padding-left:16px}.docs-field-head{flex-direction:column}.docs-field-card dl{grid-template-columns:1fr}}
</style>
