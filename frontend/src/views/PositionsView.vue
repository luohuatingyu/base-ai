<template>
  <div class="panel">
    ...
    <el-table :data="rows" table-layout="auto"><el-table-column prop="code" :label="t('common.code')" min-width="160"/><el-table-column prop="name" :label="t('common.name')" min-width="180"/><el-table-column prop="sortOrder" :label="t('common.sort')" width="110"/><el-table-column :label="t('common.operation')" width="180" fixed="right"><template #default="s"><div class="table-actions"><el-button link type="primary" @click="open(s.row)">{{ t('common.edit') }}</el-button><el-button link type="danger" @click="remove(s.row)">{{ t('common.delete') }}</el-button></div></template></el-table-column></el-table>
