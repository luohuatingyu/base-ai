        ...
        <el-tab-pane v-if="auth.hasPermission('system:task:view')" label="查进度" name="progress">
          <div v-loading="progressLoading" class="progress-panel">
            <div class="progress-toolbar">
              <div>
                <span>目标接口返回的任务编号</span>
                <code v-if="progressJobId" class="job-id-chip">{{ progressJobId }}</code>
                <small v-else>临时测试或正式执行返回 jobId 后自动显示，无需手工输入。</small>
              </div>
              <el-button :disabled="!progressJobId" @click="queryProgress">刷新进度与日志</el-button>
            </div>

            <el-alert v-if="progressError" :title="progressError" type="error" :closable="false" show-icon/>
            <el-empty v-if="!progressJobId" description="目标接口响应中尚未识别到任务编号"/>
            <template v-else-if="progressDetail">
              <el-descriptions :column="2" border class="progress-details">
                ...
                <el-descriptions-item label="Python 子任务">{{progressDetail.pythonJobs?.length||0}}</el-descriptions-item>
              ...
              <div v-if="progressDetail.pythonJobs?.length" class="progress-section">
                <h4>Python 子任务</h4>
                <el-table :data="progressDetail.pythonJobs" size="small" max-height="220">
                  <el-table-column prop="python_job_id" label="子任务编号" min-width="220"/>
              ...
              <div class="progress-section">
                <h4>统一任务日志 <span>{{progressLogs.length}} 条</span></h4>
                <el-table :data="progressLogs" size="small" max-height="320" empty-text="暂无任务日志">
...
import { extractJobId, fetchTaskProgress, resolveActiveTab } from '../utils/apiTrigger'
...
const activeTab=ref('basic'),progressJobId=ref(''),progressLoading=ref(false),progressDetail=ref(null),progressLogs=ref([]),progressError=ref(''),progressLoadedJobId=ref('')
/** 查询接口触发配置。 */
...
/** 根据目标接口响应重置自动识别的任务进度上下文。 */
function resetProgress(responseBody){
  progressJobId.value=extractJobId(responseBody)
  progressDetail.value=null
  progressLogs.value=[]
  progressError.value=''
  progressLoadedJobId.value=''
}
/** 保存调用结果，并在进度页已打开时自动加载任务详情与日志。 */
function setExecutionResult(data){
  result.value=data||{}
  resetProgress(result.value.responseBody)
  if(activeTab.value==='progress'&&progressJobId.value&&auth.hasPermission('system:task:view'))queryProgress()
}
/** 查询目标接口返回任务编号对应的平台进度与统一日志。 */
async function queryProgress(){
  if(!progressJobId.value)return
  progressLoading.value=true
  progressError.value=''
  try{
    const progress=await fetchTaskProgress(http,progressJobId.value)
    progressDetail.value=progress.detail
    progressLogs.value=progress.logs
    progressLoadedJobId.value=progressJobId.value
  }catch(e){
    progressDetail.value=null
    progressLogs.value=[]
    progressLoadedJobId.value=''
...
watch(activeTab,tab=>{
  if(tab==='progress'&&progressJobId.value&&progressLoadedJobId.value!==progressJobId.value&&auth.hasPermission('system:task:view'))queryProgress()
})
onMounted(load)
</script>
