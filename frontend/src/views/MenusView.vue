...
<script setup>
import { computed,onMounted,reactive,ref } from 'vue'; import { ElMessage,ElMessageBox } from 'element-plus'; import { useI18n } from 'vue-i18n'; import http,{showHttpError} from '../api/http'; import { useAuthStore } from '../stores/auth'; import { localizeMenuName } from '../utils/navigation'; import { buildTree } from '../utils/tree'
...
async function load(){rows.value=(await http.get('/system/menus')).data} /** 打开菜单编辑窗口。 */ function open(row,parentId){Object.assign(form,row?{...row}:empty(parentId));visible.value=true} /** 保存菜单节点。 */ async function save(){try{form.id?await http.put(`/system/menus/${form.id}`,form):await http.post('/system/menus',form);visible.value=false;await load();ElMessage.success(t('common.successSaved'))}catch(e){showHttpError(e,'common.saveFailed')}} /** 删除菜单节点。 */ async function remove(row){await ElMessageBox.confirm(t('common.confirmDelete',{name:localizedName(row)}),t('common.deleteConfirm'),{type:'warning'});await http.delete(`/system/menus/${row.id}`);await load();ElMessage.success(t('common.successDeleted'))} onMounted(load)
</script>
