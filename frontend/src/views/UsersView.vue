<template>
  <div class="panel">
    ...
    <el-table :data="rows" table-layout="auto"><el-table-column prop="username" :label="t('common.account')" min-width="170"/><el-table-column :label="t('common.name')" min-width="180"><template #default="s">{{ localizeUserDisplayName(s.row, roles, t) }}</template></el-table-column><el-table-column :label="t('users.department')" min-width="180"><template #default="s">{{ localizeDepartmentName(departments.find(d => d.id === s.row.departmentId), t) }}</template></el-table-column><el-table-column :label="t('common.status')" width="110"><template #default="s"><el-tag :type="s.row.enabled?'success':'info'">{{ s.row.enabled?t('common.enabled'):t('common.disabled') }}</el-tag></template></el-table-column><el-table-column :label="t('common.operation')" width="180" fixed="right"><template #default="s"><div class="table-actions"><el-button v-if="auth.hasPermission('system:user:update')" link type="primary" @click="open(s.row)">{{ t('common.edit') }}</el-button><el-button v-if="auth.hasPermission('system:user:delete')" link type="danger" @click="remove(s.row)">{{ t('common.delete') }}</el-button></div></template></el-table-column></el-table>
