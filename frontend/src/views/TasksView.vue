<template>
  <div class="panel">
    ...
    <el-drawer v-model="logVisible" title="链路日志" size="68%"><div v-for="item in logs" :key="item.id" class="log-line"><span>{{item.logged_at}}</span><b :class="item.level">{{item.level}}</b><em>{{item.source}}</em><code>{{item.message}}</code></div><el-empty v-if="!logs.length" description="暂无日志"/></el-drawer>
...
/** 加载任务列表和筛选选项。 */
async function load(){const params=Object.fromEntries(Object.entries(query).filter(([,v])=>v)); const [traces,types,entries]=await Promise.all([http.get('/system/tasks',{params}),http.get('/system/tasks/task-types'),http.get('/system/tasks/trigger-entries')]);rows.value=traces.data;taskTypes.value=types.data;triggerEntries.value=entries.data}
...
/** 展示完整任务快照。 */
async function showDetail(row){detail.value=(await http.get(`/system/tasks/${row.trace_id}`)).data;detailVisible.value=true}
/** 查询跨服务链路日志。 */
async function showLogs(traceId){logs.value=(await http.get(`/system/tasks/${traceId}/logs`)).data;logVisible.value=true}
/** 请求协作取消任务。 */
async function cancelTrace(traceId){const {value}=await ElMessageBox.prompt('请输入取消原因','取消任务',{inputValue:'用户请求取消'});await http.post(`/system/tasks/${traceId}/cancel`,{reason:value});ElMessage.success('已发送取消请求');load()}
/** 管理员强制中断任务。 */
async function forceTrace(traceId){await ElMessageBox.confirm('强制终止可能导致外部请求仍在处理中，是否继续？','强制终止',{type:'warning'});await http.post(`/system/tasks/${traceId}/force-terminate`,{reason:'管理员强制终止'});ElMessage.success('任务已终止');load()}
onMounted(load)
</script>
