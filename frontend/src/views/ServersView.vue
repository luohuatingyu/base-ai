<template>
  <div class="panel">
    <div class="section-head"><div><h2>{{ t('servers.title') }}</h2><p>{{ t('servers.description') }}</p></div><div class="table-actions"><el-button @click="load">{{ t('common.refresh') }}</el-button><el-button v-if="auth.hasPermission('operations:server:create')" type="primary" @click="open()">{{ t('common.add') }}</el-button></div></div>
    <el-alert :title="t('servers.securityNotice')" type="warning" show-icon :closable="false" />
    <el-table :data="rows" v-loading="loading" class="servers-table"><el-table-column prop="name" :label="t('servers.name')" min-width="180"/><el-table-column prop="mode" :label="t('servers.mode')" width="110"/><el-table-column prop="host" :label="t('servers.host')" min-width="180"/><el-table-column :label="t('common.status')" width="100"><template #default="scope">{{ scope.row.enabled ? t('common.enabled') : t('common.disabled') }}</template></el-table-column><el-table-column prop="lastTestStatus" :label="t('servers.testStatus')" width="140"/><el-table-column :label="t('common.operation')" width="390" fixed="right"><template #default="scope"><div class="table-actions"><el-button v-if="auth.hasPermission('operations:server:test')" link type="success" :disabled="!scope.row.enabled" @click="test(scope.row)">{{ t('servers.test') }}</el-button><el-button v-if="auth.hasPermission('operations:server:update')" link type="primary" @click="open(scope.row)">{{ t('common.edit') }}</el-button><el-button v-if="auth.hasPermission('operations:server:deploy')" link type="warning" :disabled="!scope.row.enabled" @click="deploy(scope.row)">{{ t('servers.deploy') }}</el-button><el-button v-if="auth.hasPermission('operations:server:logs')" link @click="history(scope.row)">{{ t('servers.history') }}</el-button><el-button v-if="auth.hasPermission('operations:server:delete')" link type="danger" @click="remove(scope.row)">{{ t('common.delete') }}</el-button></div></template></el-table-column></el-table>
