export default {
  ...
  routes: { title: '能力路由', description: '按供应商池、能力和思考等级筛选模型。', add: '新增路由', edit: '编辑路由', editSyncNotice: '编辑能力路由后，请点击“同步”；同步完成后，配置才会生效。', featureCode: '功能编码', candidates: '候选模型', providerPool: '供应商池', thinking: '思考模式', thinkingLevel: '思考等级', example: '例如 chat', sync: '同步', syncRoute: '同步路由「{name}」', syncScopeHint: '将测试当前路由已配置的全部模型供应，并把数据库中的最新路由配置同步到内存。', startSync: '开始同步', syncCompleted: '同步完成', syncDuration: '耗时 {duration}ms', syncFailed: '同步失败', noModelsToSync: '当前路由未配置模型供应，已同步成功', removeModelProvider: '删除当前模型供应', removeProviderConfirm: '确认从能力路由「{route}」删除供应商「{provider}」及其全部模型供应？供应商和模型主数据不会删除。', providerRemoved: '当前模型供应已删除' },
