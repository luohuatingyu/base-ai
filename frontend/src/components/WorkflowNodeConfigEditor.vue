    ...
    <div v-if="fields.length" class="workflow-config-grid">
      <article v-for="field in fields" :key="field.key" class="workflow-config-card"
        ...
        <div class="workflow-config-card-head-layout">
          <button type="button" class="workflow-config-card-head" :aria-expanded="openFields.includes(field.key)" @click="toggleField(field.key)">
            <span>
            ...
            <span class="workflow-config-toggle-hint">
              <span>{{ t(openFields.includes(field.key) ? 'workflowConfig.collapseValue' : 'workflowConfig.expandValue') }}</span>
              <span class="workflow-config-toggle-chevron" :class="{ open: openFields.includes(field.key) }" aria-hidden="true"></span>
            </span>
          </button>
        ...
        <div v-if="openFields.includes(field.key)" class="workflow-config-card-body">
          <fieldset class="workflow-config-controls" :disabled="fieldDisabled(field)">
            ...
            <template v-else-if="field.editor === 'modelRoute'">
              <el-select :model-value="fieldValue(field)" filterable clearable fit-input-width :loading="routeOptionsLoading"
                         :placeholder="t('workflowConfig.selectModelRoute')" :no-data-text="t('workflowConfig.noModelRoutes')"
                         @update:model-value="setFeatureCode">
                <template #label="{ label }"><el-text class="workflow-model-option-label" truncated>{{ label }}</el-text></template>
                <el-option v-for="option in routeOptions" :key="option.id"
                           :label="workflowRouteOptionLabel(option)" :value="option.featureCode">
                  <el-text class="workflow-model-option-label" truncated>{{ workflowRouteOptionLabel(option) }}</el-text>
                </el-option>
              </el-select>
              <el-alert v-if="routeOptionsError" :title="t('workflowConfig.routeOptionsFailed')" type="error" show-icon :closable="false" />
            </template>
    ...
    <section class="workflow-extra-config">
      ...
      <article v-for="key in extraKeys" :key="key" class="workflow-config-card configured">
        <button type="button" class="workflow-config-card-head" :aria-expanded="openExtra.includes(key)" @click="toggleExtra(key)">
          ...
          <span class="workflow-config-toggle-hint">
            <span>{{ t(openExtra.includes(key) ? 'workflowConfig.collapseValue' : 'workflowConfig.expandValue') }}</span>
            <span class="workflow-config-toggle-chevron" :class="{ open: openExtra.includes(key) }" aria-hidden="true"></span>
          </span>
        </button>
...
const routeOptionsLoaded = ref(false)
const routeOptionsError = ref(false)
...
watch([hasRouteSelector, routeOptions], () => {
  ...
  if (!routeOptionsLoaded.value || !hasField('featureCode')) return
  const compatibleCode = compatibleWorkflowRouteCode(routeOptions.value, config.value.featureCode)
  if (compatibleCode === null) removeField('featureCode')
  else if (compatibleCode !== config.value.featureCode) setFeatureCode(compatibleCode)
})
...
.workflow-config-toggle-hint { display: inline-flex; flex: 0 0 auto; align-items: center; gap: 7px; color: var(--el-color-primary); font-size: 12px; font-weight: 500; white-space: nowrap; }
.workflow-config-toggle-chevron { width: 7px; height: 7px; border-right: 1.5px solid currentcolor; border-bottom: 1.5px solid currentcolor; transform: rotate(45deg); transition: transform .2s ease; }
.workflow-config-toggle-chevron.open { transform: rotate(225deg); }
