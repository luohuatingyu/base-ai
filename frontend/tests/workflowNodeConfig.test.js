...
test('发布提示覆盖固定必填、方案条件、操作条件和参数数量', () => {
  ...
  assert.deepEqual(missingNodeConfigRequirements('CSV', {}), ['value'])
  assert.deepEqual(missingNodeConfigRequirements('CSV', { operation: 'INVALID', value: 'a,b' }), ['operation'])
  assert.deepEqual(missingNodeConfigRequirements('CSV', { operation: 'PARSE', value: 'a,b' }), [])
