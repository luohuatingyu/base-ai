...
test('容器名称使用平台简称的小写项目名前缀', async () => {
  ...
  assert.doesNotMatch(envExample, /^API_TRIGGER_TRUSTED_CA_DIR=/m)
...
test('仅 Caddy 暴露 HTTP 和 HTTPS 端口并持久化内部 CA', async () => {
  ...
  assert.match(serviceBlock(compose, 'caddy', null), /caddy-public-ca:\/public-ca/)
  assert.match(serviceBlock(compose, 'backend', 'python-worker'), /caddy-public-ca:\/app\/caddy-public-ca:ro/)
  assert.doesNotMatch(serviceBlock(compose, 'backend', 'python-worker'), /caddy-data:/)
  assert.match(serviceBlock(compose, 'backend', 'python-worker'), /API_TRIGGER_CADDY_CA_FILE:\s*\/app\/caddy-public-ca\/root\.crt/)
  assert.doesNotMatch(serviceBlock(compose, 'backend', 'python-worker'), /API_TRIGGER_TRUSTED_CA/)
  assert.match(compose, /^\s{2}caddy-public-ca:\s*$/m)
...
test('全部运行时镜像使用非 root 用户和最小 Linux 权限', async () => {
  ...
  assert.match(backendDockerfile, /chown appuser:appuser \/app\/caddy-public-ca/)
  assert.match(caddyDockerfile, /chown -R appuser:appuser \/data \/config \/public-ca/)
  assert.match(caddyEntrypointSource, /base-ai-ip-cert.*\\?\s*--serve/s)
  assert.match(caddyEntrypointSource, /publish_public_root_certificate/)
  assert.match(caddyEntrypointSource, /\/public-ca\/root\.crt/)
...
test('自建镜像使用必填 Git revision 标签并写入 OCI 元数据', async () => {
  ...
  const testWorkflow = await readFile(new URL('.github/workflows/test.yml', root), 'utf8')
  const scanWorkflow = await readFile(new URL('.github/workflows/security-scan.yml', root), 'utf8')
  ...
  assert.match(scanWorkflow, /--build-arg "APP_IMAGE_REVISION=\$APP_IMAGE_REVISION"/)
})
