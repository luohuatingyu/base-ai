...
class TraceRuntimeRegistry:
    ...
    async def remove(self, trace_id: str) -> None:
        """移除已完成的异步任务。"""
        async with self._lock:
...
class JavaTraceReporter:
    ...
    async def heartbeat(self, python_trace_id: str) -> None:
        """任务运行期间每十五秒发送一次心跳。"""
        try:
            while True:
                await asyncio.sleep(15)
                await self.report(python_trace_id, "RUNNING")
