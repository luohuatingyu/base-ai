...
- **设备 Agent：** `DEVICE_AGENT_NONCE_TTL_SECONDS`、`DEVICE_AGENT_SIGNATURE_CLOCK_SKEW_SECONDS`、`DEVICE_AGENT_HEARTBEAT_STALE_SECONDS`、`DEVICE_AGENT_PAIRING_TTL_SECONDS`、`DEVICE_AGENT_REGISTRY_DEFAULT_PORT`，以及固定运行时版本 `AGENT_NODE_VERSION`、`AGENT_APPIUM_SPEC`、`AGENT_XCUITEST_SPEC`。
...
- 服务器在管理页面中手工新增和维护，新增记录默认使用 SSH 模式。具有服务器测试权限的用户可打开“资源监控”弹窗实时查询 CPU、系统负载、内存、磁盘和运行时长；监控快照不持久化，不查询 Docker 容器，也不需要访问 Docker Socket。
- 在“服务器管理 → 秘钥管理”集中保存标签、RSA 类型、public key、private key、certificate、账号、密码和私钥口令；支持仅保存账号密码。新建 SSH 服务器从同一所有者的启用凭据中选择，账号自动带入；密钥无账号时可在服务器填写。历史服务器保留原配置，可编辑切换至共享凭据。修改凭据后所有引用服务器的后续连接使用最新值；引用中的凭据不能删除或停用。列表不返回秘密，明文查看仅限管理员。敏感字段留空保留原值；需要移除秘密时新建凭据并切换引用。密钥与证书材料每项限 32 KiB；certificate 目前仅保存，不参与 SSH 证书认证，不自动生成密钥或校验密钥配对。
