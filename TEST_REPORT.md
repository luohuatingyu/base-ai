...
- Commit: 0679f70
- 提交说明: Add independent loopback access control
- 测试日期: 2026-07-25
- 分支: master
...
- Host 白名单、回环开关和其他私网开关保存到现有系统参数表，并通过 Redis 缓存实现保存后立即生效。
- 未保存配置时额外 Host 白名单为空，回环开关默认开启，其他私网开关默认关闭。
- `localhost`、`127.0.0.1` 和 `::1` 无需加入 Host 白名单；读取旧配置时会自动从页面值中剔除。
- 支持精确 Host、`*.example.com` 和 `*`；`*` 仅放开 Host，不绕过回环和私网两个独立开关。
- 通用系统参数页面隐藏并禁止修改三个专用安全配置键。
- 私网阻止逻辑补充 IPv6 ULA（`fc00::/7`）识别。

## 📋 本次变更测试结果（2026-07-25）

**变更范围**：接口触发动态 Host 白名单、独立回环开关、其他私网访问策略、专用配置页面及 SSRF 安全边界。
...
**实现与验证确认**：
- 后端定向测试覆盖额外 Host 默认空、回环默认开启、其他私网默认关闭、旧回环值清理、三个开关缓存失效、回环关闭优先级和动态生效。
- 前端定向测试覆盖独立回环开关、额外 Host 占位符、默认值、加载/保存接口、星号风险提示和完全开放二次确认。
- 第一次后端定向执行共 16 项，其中 1 项因策略无条件解析不存在的示例域名失败；修正为仅在回环或私网限制开启时执行 DNS 地址分类后，最终 17/17 通过。
...
| 测试范围 | 执行命令 | 结果 |
| --- | --- | --- |
| 后端定向测试（修正前） | `docker run --rm -v "$PWD/backend:/workspace" -v "$HOME/.m2:/root/.m2" -w /workspace maven:3.9.9-eclipse-temurin-17 mvn -B -Dtest=ApiTriggerSecurityConfigurationServiceTest,ApiTriggerUrlPolicyTest,ApiTriggerSecurityConfigurationControllerTest,SystemConfigurationServiceTest test` | 16 项中 1 项失败；原因是两个网络开关均开启时仍执行 DNS 解析 |
| 后端定向测试（最终） | 同上 | 17 通过，0 失败，0 错误，0 跳过 |
| 前端定向测试 | `node --test frontend/test/api-trigger-security.test.mjs frontend/test/api-trigger.test.mjs frontend/test/navigation.test.mjs` | 20 通过，0 失败，0 跳过 |
| 后端完整测试 | `docker run --rm -v "$PWD/backend:/workspace" -v "$HOME/.m2:/root/.m2" -w /workspace maven:3.9.9-eclipse-temurin-17 mvn test -B` | 71 通过，0 失败，0 错误，0 跳过 |
| 前端完整回归 | `node --test frontend/test/*.test.mjs` | 56 通过，0 失败，0 跳过 |
...
- 正常场景：默认无需白名单即可访问三个回环 Host；精确 Host、子域通配、任意 Host 星号、回环和其他私网组合均按独立开关通过。
- 边界场景：额外 Host 空列表仅保留隐式回环访问；重复规则去重；旧配置中的三个内置回环值自动剔除；大小写、空白和 IPv6 方括号被规范化。
- 异常场景：非法 URL、带协议或端口的 Host 规则、非法通配、越界 IPv4、非 HTTP/HTTPS URL 均被拒绝。
- 权限与安全：查看和更新使用独立权限；通用参数入口无法修改三个专用键；关闭回环后即使配置 `*` 也拒绝回环；关闭其他私网时拒绝 RFC1918、链路本地、组播和 IPv6 ULA 地址。
...
- 运行环境：Compose 已基于提交 `0679f70` 重新构建，Backend、Frontend、Python Worker 均处于 healthy 状态。

## 🔄 重测触发条件

- 修改 `backend/src/main/java/` 下接口触发 URL 校验、运行时配置读取、缓存失效或系统参数隔离逻辑。
- 修改接口触发安全配置 Controller、权限编码、菜单初始化或前端专用配置页面。
- 修改 Host 规则格式、回环默认值、私网地址识别范围或 `*` 的语义。
- 修改 Redis 缓存键、系统参数固定键、配置持久化方式或 Compose 运行配置。
- 用户明确要求重新执行完整覆盖测试。

## ⚠️ 已知问题与限制

- `*` 配合开启回环和其他私网会允许任意 HTTP/HTTPS Host，页面已显示警告并要求二次确认，但管理员仍需自行评估 SSRF 风险。
