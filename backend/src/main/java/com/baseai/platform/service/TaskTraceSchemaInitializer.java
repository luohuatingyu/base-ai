...
public class TaskTraceSchemaInitializer implements ApplicationRunner {
    ...
    public TaskTraceSchemaInitializer(@Qualifier("mysqlJdbcTemplate") JdbcTemplate jdbcTemplate) { this.jdbcTemplate = jdbcTemplate; }
    ...
    public void run(ApplicationArguments arguments) {
        ...
        columns.forEach((name, type) -> {
            Integer count = jdbcTemplate.queryForObject("""
                ...
                """, Integer.class, name);
            if (count != null && count == 0) jdbcTemplate.execute("ALTER TABLE task_trace ADD COLUMN " + name + " " + type);
        });
        jdbcTemplate.execute("""
            CREATE TABLE IF NOT EXISTS task_trace_python (
                python_trace_id VARCHAR(64) PRIMARY KEY, parent_trace_id VARCHAR(32) NOT NULL,
